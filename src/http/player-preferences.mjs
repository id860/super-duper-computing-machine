...
const SHOP = [
	{ key: 'frame_neon', title: 'Неоновая рамка профиля', price: 250, type: 'cosmetic', slot: 'frame' },
	{ key: 'frame_aurora', title: 'Рамка «Аврора»', price: 360, type: 'cosmetic', slot: 'frame' },
	{ key: 'frame_forest', title: 'Рамка «Чаща»', price: 300, type: 'cosmetic', slot: 'frame' },
	{ key: 'frame_crimson', title: 'Рамка «Багрянец»', price: 420, type: 'cosmetic', slot: 'frame' },
	{ key: 'nick_gradient', title: 'Градиентный ник', price: 400, type: 'cosmetic', slot: 'nick' },
	{ key: 'nick_gold', title: 'Золотой ник', price: 520, type: 'cosmetic', slot: 'nick' },
	{ key: 'nick_ice', title: 'Ледяной ник', price: 340, type: 'cosmetic', slot: 'nick' },
	{ key: 'nick_mint', title: 'Мятный ник', price: 300, type: 'cosmetic', slot: 'nick' },
	{ key: 'badge_pioneer', title: 'Значок пионера', price: 150, type: 'cosmetic', slot: 'badge' },
	{ key: 'badge_creator', title: 'Значок творца', price: 300, type: 'cosmetic', slot: 'badge' },
	{ key: 'badge_architect', title: 'Значок архитектора', price: 460, type: 'cosmetic', slot: 'badge' },
	{ key: 'badge_guardian', title: 'Значок хранителя', price: 520, type: 'cosmetic', slot: 'badge' },
	{ key: 'trail_spark', title: 'Искристый след', price: 280, type: 'cosmetic', slot: 'trail' },
	{ key: 'trail_rainbow', title: 'Радужный след', price: 380, type: 'cosmetic', slot: 'trail' },
	{ key: 'trail_ember', title: 'Углистый след', price: 330, type: 'cosmetic', slot: 'trail' },
	{ key: 'cursor_comet', title: 'Курсор-комета', price: 220, type: 'cosmetic', slot: 'cursor' },
	{ key: 'cursor_pixel', title: 'Курсор-пиксель', price: 190, type: 'cosmetic', slot: 'cursor' },
	{ key: 'cursor_halo', title: 'Курсор-ореол', price: 260, type: 'cosmetic', slot: 'cursor' },
...
// Owned cosmetics with their slot and title, so the profile can render a
// wardrobe without duplicating the catalogue on the client.
function wardrobe(user) { const items = user?.inventory?.items || {}; const out = []; for (const item of SHOP) { if (item.type !== 'cosmetic' || !items[item.key]) continue; out.push({ key: item.key, title: item.title, slot: item.slot, count: Number(items[item.key]) || 1 }); } return out; }
export { equipped as equippedCosmetics, wardrobe as ownedCosmetics, SHOP as shopOffers };
export async function handlePlayerExperienceRequest(req, res, ctx, store) {
	...
	if (req.method === 'GET' && url.pathname === '/api/me/preferences') { const user = requireUser(ctx); ok(res, { preferences: user.preferences || {}, cosmetics: { equipped: equipped(user), owned: wardrobe(user) } }); return true; }
	...
	if (req.method === 'POST' && url.pathname === '/api/me/cosmetics') { const user = requireUser(ctx), body = await readJson(req), item = byKey.get(body.key), slot = body.slot; if (!item || item.type !== 'cosmetic' || item.slot !== slot || !user.inventory?.items?.[item.key]) throw Object.assign(new Error('Предмет не куплен'), { status: 403 }); user.cosmetics ||= {}; const active = equipped(user); user.cosmetics.equipped = { ...active, [slot]: active[slot] === item.key ? null : item.key }; store.schedule(); ok(res, { cosmetics: { equipped: equipped(user), owned: wardrobe(user) } }); return true; }
