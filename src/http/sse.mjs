// Replayable Server-Sent Events with bounded per-client backpressure queues.
export function createSse({ heartbeatMs = 25000, maxPerWorld = 500, historyLimit = 1000, maxQueueBytes = 256 * 1024 } = {}) {
	...
	function stateFor(res) {
		...
		if (!state) { state = { blocked: false, queue: [], bytes: 0, closed: false }; clients.set(res, state); }
		return state;
	}

	function closeSlowClient(set, res, state) {
		state.closed = true; state.queue.length = 0; state.bytes = 0;
		set?.delete(res);
		try { res.end(); } catch {}
		return false;
	}
	...
	function flush(set, res) {
		...
		while (state.queue.length) {
			const payload = state.queue.shift(); state.bytes -= Buffer.byteLength(payload);
			let writable = false;
			try { writable = res.write(payload); } catch { closeSlowClient(set, res, state); return; }
			if (!writable) { state.blocked = true; res.once?.('drain', () => flush(set, res)); return; }
		}
	}
	...
	function send(set, res, payload) {
		...
		if (state.blocked) {
			...
			if (state.bytes + bytes > maxQueueBytes) return closeSlowClient(set, res, state);
			state.queue.push(payload); state.bytes += bytes; return true;
		}
		try {
			if (!res.write(payload)) { state.blocked = true; res.once?.('drain', () => flush(set, res)); }
			return true;
		} catch { return closeSlowClient(set, res, state); }
	}

	const timer = setInterval(() => {
		for (const set of channels.values()) for (const res of set) send(set, res, ': ping\n\n');
	}, heartbeatMs);
	timer.unref?.();
	...
	function subscribe(worldId, req, res) {
		...
		set.add(res); stateFor(res);
		const lastId = String(req.headers?.['last-event-id'] || '').trim();
		if (lastId) {
			const history = histories.get(worldId) || [], index = history.findIndex((entry) => entry.id === lastId);
			if (index >= 0) for (const entry of history.slice(index + 1)) send(set, res, entry.payload);
			else send(set, res, encode('resync', { reason: 'history-miss' }));
		}
		broadcastPresence(worldId);
		let cleaned = false;
		const cleanup = () => { if (cleaned) return; cleaned = true; const state = stateFor(res); state.closed = true; set.delete(res); broadcastPresence(worldId); };
		req.on('close', cleanup); req.on('error', cleanup);
	}

	function broadcastPresence(worldId) {
		const set = channels.get(worldId); if (!set?.size) return;
		const payload = encode('presence', { online: online(worldId) });
		for (const res of [...set]) send(set, res, payload);
	}
	...
	function broadcast(worldId, event, data) {
		...
		if (set?.size) for (const res of [...set]) send(set, res, payload);
		return id;
	}
