...
import { checkCsrf, fail, getSession, isHttps, securityHeaders, serveStatic } from './src/http/kit.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const PORT = Number(process.env.PORT || 3000), DATA_DIR = process.env.DATA_DIR || join(__dirname, 'data');
const PUBLIC_DIR = join(__dirname, 'public'), APP_ORIGIN = process.env.APP_ORIGIN || '';
...
async function bootstrap() {
	...
	const tick = () => { try { const result = runAutomation(db, { emit: (worldId, event, data) => sse.broadcast(worldId, event, data), notify }); if (result?.changed) store.schedule(0); } catch (error) { console.error('automation tick failed:', error); } };
	tick(); const automationTimer = setInterval(tick, AUTOMATION_INTERVAL_MS); automationTimer.unref?.();
	const server = createServer(async (req, res) => {
		const https = isHttps(req); securityHeaders(res, { https });
		try {
