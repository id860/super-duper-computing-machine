...
import { handleChunkRequest } from './src/http/chunks.mjs';
...
async function bootstrap() {
	...
	if (adminNick && adminPassword) {
		if (adminPassword.length < 12 || adminPassword.length > 128) console.warn('ADMIN_PASSWORD должен быть 12..128 символов, админ не создан.');
			...
			} else { admin.role = 'admin'; admin.password = hashPassword(adminPassword); }
			store.schedule(0);
		}
	}
	const sse = createSse();
	...
	const tick = () => {
		try { const result = runAutomation(db, { emit: (worldId, event, data) => sse.broadcast(worldId, event, data), notify }); if (result?.changed) store.schedule(0); }
		catch (error) { console.error('automation tick failed:', error); }
	};
	tick();
	...
	const server = createServer(async (req, res) => {
		...
		try {
			if (req.url.startsWith('/api/')) {
				...
				if (await handleChunkRequest(req, res, ctx, db)) return;
	...
	const shutdown = async (signal) => {
		console.log(`\n${signal}: завершение...`);
		clearInterval(automationTimer); sse.close(); server.close();
		try { await store.flush(); } catch (error) { console.error(error); }
		process.exit(0);
	};
