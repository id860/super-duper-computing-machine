...
import { touchChunk } from './chunk-lru.js';
...
PixelEngine.prototype._loadViewportChunks = async function () {
	...
	for (const key of this._loadedChunks) {
		if (!before.has(key) || !this._chunkAccess.has(key)) {
			const evicted = touchChunk(this._chunkAccess, key, stamp, MAX_TRACKED_CHUNKS);
			for (const oldKey of evicted) this._loadedChunks.delete(oldKey);
		}
	}
};
